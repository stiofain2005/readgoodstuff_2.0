...
/* The allow code is not needed as only the server can add/edit posts */
Posts.allow({
...
Meteor.methods({
    ...
    postInsert: function(newPost){
        ...
        if(newPost.publish) {
            ...
            // else set the publish Date to current date
        ...
        var post = _.extend(newPost, {
            submitted: new Date(),
            // this will be undefined if publish not picked
    ...
    postUpdate:function(currentPostId, postAttributes, publishDate){
        ...
        if(postAttributes.publish) {
            ...
            // if exists and published and havent looked up itself
            ...
            // set a publish date if not already set
